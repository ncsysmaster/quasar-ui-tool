...
`postViewState()`는 현재 모델, 선택 ID, 문서 존재 여부를 각 Activity Bar View에 전달한다.
...
- Extension에 포함된 `vscode-extension/generator/generate-vue.mjs`를 실행한다.
...
- Screen, Script, DataSet 탭과 Screen 도구 모음
- JSON을 Vue/Quasar VNode으로 변환하는 런타임 렌더러
- 선택, 더블클릭, 직접 Text 편집, DnD, 키보드 삭제 및 클립보드
- Form Search Grid 컨텍스트 메뉴, 행/열 크기 조절, 셀 나누기 대화상자
- Grid `col-N` 및 행 합계 배지
