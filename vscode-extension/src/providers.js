...
const { exportPageJsonToTaggedPpt } = require("./pptExportCommand");
const { listPiniaStores } = require("./piniaStoreRepository");

const { getEditorHtml } = require("./webviews");
...
class PageEditorProvider {
  ...
  async resolveCustomTextEditor(document, webviewPanel) {
    ...
    webviewPanel.webview.onDidReceiveMessage(async (message) => {
      ...
      if (message.type === "exportTaggedPpt") {
        await exportPageJsonToTaggedPpt(
          document,
          editorState.getModel(),
      ...
      if (message.type === "addComponent") {
        await editorState.addComponent(message.index);
      }

      if (message.type === "updateEvent") {
        await editorState.updateSelectedEvent(message.eventName, message.value);
        if (message.openScriptTab && message.value) {
          editorState.setEditorTab("script");
        }
      }

      if (message.type === "openEventMethod") {
        await editorState.openSelectedEventMethod(
          message.eventName,
          message.value || "",
        );
      }
      ...
      if (message.type === "deleteComponent") {
        await editorState.removeComponentById(message.id);
      }

      if (message.type === "addField") {
        await editorState.addDatasetField();
      }

      if (message.type === "updateField") {
        await editorState.updateDatasetField(
          message.index,
          message.name,
          message.value,
        );
      ...
      if (message.type === "removeField") {
        await editorState.removeDatasetField(message.index);
      }
