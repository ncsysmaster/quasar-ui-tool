...
function getEditorHtml(webview, runtimeUris) {
  ...
  return htmlShell(
    ...
    <script nonce="${nonce}">
      ...
      let tableContextInitialNode = null
      ...
      document.querySelectorAll('[data-tab]').forEach((button) => {
        button.addEventListener('click', () => {
          activateEditorTab(button.dataset.tab, { notify: true })
        })
      })
      ...
      activateEditorTab('screen', { notify: true })

      window.addEventListener('keydown', handleGlobalKeydown, true)

      function activateEditorTab(tabName, options = {}) {
        activeTab = ['screen', 'script', 'store'].includes(tabName) ? tabName : 'screen'
        document.querySelectorAll('[data-tab]').forEach((tab) => {
          tab.classList.toggle('active', tab.dataset.tab === activeTab)
        })
        updateScreenTools()
      ...
      function renderTablePreviewComponent(component, scope, repeatIndex) {
        ...
        const wrapperProps = {
          ...
          tabindex: 0,
          draggable: true,
          onVnodeMounted: (vnode) => scheduleTableHeaderDomSync(vnode.el, component),
          onVnodeUpdated: (vnode) => scheduleTableHeaderDomSync(vnode.el, component),
          onClick: (event) => {
            event.stopPropagation()
            ...
            if (headerInfo) {
              ...
              if (event.ctrlKey || event.metaKey) {
                event.preventDefault()
                if (handleTableHeaderCtrlClick(component, headerInfo)) {
                  markSelectedTableHeaderDomSelection(event.currentTarget, component)
                }
                return
              }
              selectedTableHeaderMergeCells = [createTableHeaderSelectionCell(component, headerInfo)]
              markSelectedTableHeaderDomSelection(event.currentTarget, component)
            }
            ...
            if (bodyInfo) {
              ...
              if (event.ctrlKey || event.metaKey) {
                ...
                markSelectedTableBodyDomSelection(event.currentTarget, component)
                return
              }
              selectedTableBodyMergeCells = [createTableBodySelectionCell(component, bodyInfo)]
              markSelectedTableBodyDomSelection(event.currentTarget, component)
            }
          ...
          onContextmenu: (event) => {
            ...
            if (headerInfo) {
              selectedTableBodyMergeCells = []
              if (!isTableHeaderInfoSelected(component, headerInfo)) {
                selectedTableHeaderMergeCells = [createTableHeaderSelectionCell(component, headerInfo)]
              }
              scheduleTableHeaderDomSync(event.currentTarget, component)
              showTableContextMenu(event.clientX, event.clientY, component.id, {
                layout: 'header',
                initialNode: tableHeaderInfoToColumnEditorNode(headerInfo, component),
                canMergeHeader: canMergeSelectedTableHeaderCells(component),
                canSplitHeader: canSplitSelectedTableHeaderCell(component, headerInfo)
              })
              scheduleTableHeaderDomSync(event.currentTarget, component)
              return
            }
            ...
            if (bodyInfo) {
              selectedTableHeaderMergeCells = []
              if (!isTableBodyInfoSelected(component, bodyInfo)) {
                selectedTableBodyMergeCells = [createTableBodySelectionCell(component, bodyInfo)]
              }
              scheduleTableHeaderDomSync(event.currentTarget, component)
              showTableContextMenu(event.clientX, event.clientY, component.id, {
                layout: 'body',
                initialNode: tableBodyInfoToColumnEditorNode(bodyInfo, component),
                canMergeHeader: canMergeSelectedTableBodyCells(component),
                canSplitHeader: canSplitSelectedTableBodyCell(component, bodyInfo)
              })
              scheduleTableHeaderDomSync(event.currentTarget, component)
              return
            }
